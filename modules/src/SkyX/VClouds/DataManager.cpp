...
namespace SkyX { namespace VClouds
{
	...
	void DataManager::_copy3DCellArraysData(Cell ***inOR, Cell ***dest, const int& nx, const int& ny, const int& nz)
	{
		int u, v, w;
		...
		for (u = 0; u < nx; u++)
		{
			for (v = 0; v < ny; v++)
			{
				for (w = 0; w < nz; w++)
				{
					dest[u][v][w].act = inOR[u][v][w].act;
					dest[u][v][w].cld = inOR[u][v][w].cld;
					dest[u][v][w].hum = inOR[u][v][w].hum;

					dest[u][v][w].pact = inOR[u][v][w].pact;
					dest[u][v][w].pext = inOR[u][v][w].pext;
					dest[u][v][w].phum = inOR[u][v][w].phum;

					dest[u][v][w].dens = inOR[u][v][w].dens;
					dest[u][v][w].light = inOR[u][v][w].light;
				}
			}
		}
	}
