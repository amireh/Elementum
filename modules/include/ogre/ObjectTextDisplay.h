#ifndef H_ObjectTextDisplay_H
#define H_ObjectTextDisplay_H
...
public:
    ObjectTextDisplay(const Ogre::MovableObject* p, const Ogre::Camera* c) {
        ...
        m_pOverlay = Ogre::OverlayManager::getSingleton().create(p->getName() + "_overlay");
        m_pContainer =
        static_cast<Ogre::OverlayContainer*>(
          Ogre::OverlayManager::getSingleton().createOverlayElement("Panel", p->getName() + "_container"));

        m_pOverlay->add2D(m_pContainer);

        m_pText = Ogre::OverlayManager::getSingleton().createOverlayElement("TextArea", p->getName() + "_text");
        m_pText->setDimensions(1.0, 1.0);
        m_pText->setMetricsMode(Ogre::GMM_PIXELS);
        m_pText->setPosition(0, 0);

        m_pText->setParameter("font_name", "BlueHighway-12");
        ...
        m_pContainer->removeChild(m_p->getName() + "_text");
